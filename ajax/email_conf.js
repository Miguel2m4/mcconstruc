...
function contactUsSubmit() {
	...
	if( !eIsValid ) {
		...
		$(".form-result").css("display", "none").html("<p class='note error'>Por favor escriba un email valido.</p><br />").fadeIn(400);
	...
	if( comment.length == 0 || comment == '' || comment == null ) {
		$(".form-result").css("display", "none").html("<p class='note error'>Por favor escriba su mensaje.</p><br />").fadeIn(400);
		...
		if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			$(".contactForm").slideUp(1000, function() {
				...
				if( parseInt(xmlhttp.responseText) == 1 ) {
					// message sent
					$(".form-result").css("display", "none").html("<p class='note success'>Su mensaje ha sido enviado!</p><br />").fadeIn('fast');
				} else {
					// message failed to send
					$(".form-result").css("display", "none").html("<p class='note error'>Lo sentimos, pero su mensaje no pudo ser enviado, por favor trate mas tarde! Error: "+xmlhttp.responseText+"</p>").fadeIn('fast');
				}
